...
function renderFinance(data) {
  ...
  data.forEach((item) => {
    ...
    tr.innerHTML = `
            ...
            <td>
                <button class="btn btn-sm ${item.is_paid ? "btn-outline-warning" : "btn-outline-success"} toggle-payment" data-id="${item.referee_id}">
                    <i class="fas ${item.is_paid ? "fa-undo" : "fa-check"}"></i>
                    ${item.is_paid ? "إلغاء الدفع" : "تسديد"}
                </button>
            </td>
        `;
  ...
  document.querySelectorAll(".toggle-payment").forEach((btn) => {
    btn.addEventListener("click", () => togglePayment(btn.dataset.id));
  });
}
...
async function togglePayment(refereeId) {
  try {
    // Find all matches for this referee and mark as paid
    const { data: matchesData, error } = await supabase
      .from("matches")
      .select("id")
      .or(
        `main_referee_id.eq.${refereeId},fourth_referee_id.eq.${refereeId},assistant1_referee_id.eq.${refereeId},assistant2_referee_id.eq.${refereeId}`,
      )
      .eq("is_paid", false);
    ...
    if (matchesData.length === 0) {
      Swal.fire({
        icon: "info",
        title: "معلومات",
        text: "لا توجد مباريات غير مدفوعة لهذا الحكم",
        confirmButtonText: "حسناً",
      });
      return;
    }
    ...
    Swal.fire({
      icon: "success",
      title: "تم التسديد",
      text: `تم تسديد ${matchesData.length} مباراة لهذا الحكم`,
      timer: 2000,
    ...
    Swal.fire({
      icon: "error",
      title: "خطأ",
      text: error.message || "حدث خطأ في تحديث حالة الدفع",
      confirmButtonText: "حسناً",
    });
  }
}
